...
function FastingHistoryGraph({ records }: FastingHistoryGraphProps) {
  ...
  const days = Object.keys(dailyData).sort((a, b) => 
    ...
    <div className="fasting-history-graph">
      ...
      <div className="graph-container">
        {days.map(day => {
          ...
          const hasCheatDay = dayRecords.some(record => 
          ...
          return (
            <div key={day} className="day-column">
              ...
              <div className={`day-bar ${!dayRecords.length ? 'empty-day' : ''} ${hasCheatDay ? 'cheat-day' : ''} ${hasSuccessfulFast ? 'successful-day' : ''}`}>
                {dayRecords.map((record, index) => {
                  ...
                  const startPercent = Math.max(0, ((startTime.getTime() - dayStart) / (24 * 60 * 60 * 1000)) * 100);
                  let endPercent = Math.min(100, ((endTime.getTime() - dayStart) / (24 * 60 * 60 * 1000)) * 100);
